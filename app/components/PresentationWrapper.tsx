  ...
  return (
    <>
      ...
      <div className="fixed right-2 md:right-6 top-1/2 -translate-y-1/2 z-50">
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-full shadow-lg p-2 md:p-3 flex flex-col gap-1.5 md:gap-2">
          {slides.map((slide) => (
            <button
              key={slide.id}
            ...
            >
              ...
              <span className="hidden md:block absolute right-full mr-3 top-1/2 -translate-y-1/2 whitespace-nowrap bg-gray-900 dark:bg-gray-700 text-white text-xs px-3 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                {slide.title}
              </span>
            </button>
          ))}
      ...
      <div className="fixed bottom-4 md:bottom-8 right-4 md:right-8 z-50 flex flex-col gap-2">
        <button
          ...
          className={`p-2 md:p-3 rounded-full bg-white dark:bg-gray-800 shadow-lg transition-all ${
            currentSlide === 0 
              ? 'opacity-30 cursor-not-allowed' 
              : 'hover:bg-blue-50 dark:hover:bg-gray-700 hover:shadow-xl'
          }`}
          aria-label="Slide anterior"
        >
          <ChevronUp size={20} className="md:w-6 md:h-6 text-blue-600 dark:text-blue-400" />
        </button>
        <button
          onClick={nextSlide}
        ...
        >
          <ChevronDown size={20} className="md:w-6 md:h-6 text-blue-600 dark:text-blue-400" />
        </button>
      </div>

      {/* Indicador de slide actual - responsive */}
      <div className="fixed bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 z-50 bg-white dark:bg-gray-800 px-3 py-2 md:px-6 md:py-3 rounded-full shadow-lg">
        <div className="flex items-center gap-2 md:gap-3">
          <span className="text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300">
            {currentSlide + 1} / {totalSlides}
          </span>
          <span className="hidden sm:inline text-xs text-gray-500 dark:text-gray-400 max-w-[120px] md:max-w-none truncate">
